;(function() {
	...
	class Scene {
		...
		static create(name, constructor) { this.scenes[name] = new Scene(constructor, name); }
		static remove(name) { this.scenes[name].remove(); }
		
		static get(name) { this.scenes[name]; }
		static run(name) { this.scenes[name].run(); }
		static exit(name) { this.scenes[name].exit(); }
	};
